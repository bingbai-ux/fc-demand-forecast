...
def analyze_forecast_accuracy_from_past_sales(store_id: str, supplier_names: List[str],
    ...
    for group in forecast_data.get("supplierGroups", []):
        ...
        for product in group.get("products", []):
            ...
            # 過去売上データを取得
            past_sales = product.get("pastSales", [])
            
            if len(past_sales) >= 4:
                # 前半と後半に分割
                half = len(past_sales) // 2
                first_half = past_sales[:half]
                second_half = past_sales[half:]
                
                # 前半の平均で後半を予測
                first_half_avg = sum([s.get("qty", 0) for s in first_half]) / len(first_half)
...
def compare_lookback_periods(store_id: str, supplier_names: List[str]) -> pd.DataFrame:
    ...
    for lookback in lookback_options:
        ...
        analysis = analyze_forecast_accuracy_from_past_sales(store_id, supplier_names, lookback)
...
def analyze_by_rank(store_id: str, supplier_names: List[str], lookback_days: int = 14) -> pd.DataFrame:
    """ランク別の精度分析"""
    analysis = analyze_forecast_accuracy_from_past_sales(store_id, supplier_names, lookback_days)
    
    if "error" in analysis:
        return pd.DataFrame()
    
    df = pd.DataFrame(analysis.get("results", []))
...
def generate_recommendations(lookback_df: pd.DataFrame, rank_df: pd.DataFrame, cv_df: pd.DataFrame) -> List[str]:
    ...
    if len(lookback_df) > 0:
        best_lookback = lookback_df.loc[lookback_df["精度率 (%)"].idxmax()]
    ...
    # ランク別の対策
    if len(rank_df) > 0:
        high_stockout_ranks = rank_df[rank_df["欠品率 (%)"].apply(lambda x: float(x) if x != "-" else 0) > 10]
        if len(high_stockout_ranks) > 0:
            ranks = ", ".join(high_stockout_ranks["ランク"].tolist())
            recommendations.append(
                f"**欠品リスク対策**: ランク{ranks}の商品は欠品率が高いため、安全在庫係数の引き上げを検討"
            )
    ...
    if len(cv_df) > 0:
        ...
        if len(unstable) > 0:
            recommendations.append(
                f"**変動が大きい商品への対策**: CV≥0.6の不安定な商品は、安全在庫係数を現在の1.0から1.5に引き上げることを検討"
            )
